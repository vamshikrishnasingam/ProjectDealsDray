...
function UserLoginStore({ children }) {
  ...
  const loginUser = async (userCredObj) => {
    await axios
      .post("http://localhost:1729/user-api/user-login", userCredObj)
      .then((response) => {
  ...
  const checkTokenAndFetchUser = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:1729/user-api/verify-token",
        null,
        {
          headers: {
      ...
      if (response.data.message === "Token is valid") {
        ...
        const userDataResponse = await axios.get(
          "http://localhost:1729/user-api/get-user-info",
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  return (
    <loginContext.Provider
      value={[currentUser, loginUser, userLoginStatus, loginErr, logoutUser]}
    >
      {children}
    </loginContext.Provider>
  );
